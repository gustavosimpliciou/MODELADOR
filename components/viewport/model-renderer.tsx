...
const _EDGES_GEO_CACHE = new Map<number, THREE.EdgesGeometry>()
function getEdgesGeo(size: number): THREE.EdgesGeometry {
  const key = Math.round(size * 100)
  let geo = _EDGES_GEO_CACHE.get(key)
  if (!geo) {
    geo = new THREE.EdgesGeometry(getPlaneGeo(size))
    _EDGES_GEO_CACHE.set(key, geo)
  }
  return geo
}
  ...
  return (
    ...
    >
      ...
      <lineSegments renderOrder={1000}>
        <primitive object={getEdgesGeo(data.diag)} attach="geometry" />
    ...
    <>
      {cuts.map((c, i) => (
        <group key={i} position={c.point.toArray()} quaternion={c.quat.toArray() as [number, number, number, number]}>
          ...
          <lineSegments renderOrder={3}>
            <primitive object={getEdgesGeo(c.diag)} attach="geometry" />
