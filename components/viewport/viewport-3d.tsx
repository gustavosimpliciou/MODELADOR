...
import { OrbitControls, Grid, AdaptiveDpr, AdaptiveEvents } from '@react-three/drei'
    ...
    (clientX: number, clientY: number) => {
      ...
      if (faceIndex === null) {
        ...
        if (c && c.face === faceIndex && c.mode === cutMode && c.angle === angle) {
          ...
          const faceCount = modelMesh.geometry.index
            ? modelMesh.geometry.index.count / 3
            : modelMesh.geometry.getAttribute("position").count / 3
          const maxHoverFaces = faceCount > 400_000 ? 80_000 : faceCount > 150_000 ? 200_000 : 2_000_000
          newHovered = smartSelect(
            modelMesh.geometry,
            faceIndex,
            { sharpAngle: angle, mode: cutMode, maxFaces: maxHoverFaces },
