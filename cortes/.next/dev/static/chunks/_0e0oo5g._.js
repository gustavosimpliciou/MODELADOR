...
function TopBar({ onExport }) {
    ...
    // Ref para o intervalo de polling (evita múltiplos polls simultâneos)
    ...
    // ── Polling de input.files ───────────────────────────────────────────────────
    // O evento 'change' pode não disparar em contextos de proxy/iframe (React 19 +
    // Next.js 16). A propriedade input.files É preenchida sempre que o usuário
    // confirma a seleção no diálogo do SO — polling garante detecção universal.
    const startPolling = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "TopBar.useCallback[startPolling]": ()=>{
            // Cancela poll anterior se existir
            if (pollRef.current) clearInterval(pollRef.current);
            ...
            pollRef.current = setInterval({
                "TopBar.useCallback[startPolling]": ()=>{
                    ...
                    if (!file) return;
                    // Arquivo detectado — parar poll e processar
                    clearInterval(pollRef.current);
                    pollRef.current = null;
                    input.value = ''; // permite re-selecionar o mesmo arquivo
                    processFile(file);
                }
            }["TopBar.useCallback[startPolling]"], 100); // checar a cada 100ms
            // Segurança: encerrar polling após 5 minutos (usuário abandonou)
            setTimeout({
                "TopBar.useCallback[startPolling]": ()=>{
                    if (pollRef.current) {
                        clearInterval(pollRef.current);
            ...
            }["TopBar.useCallback[startPolling]"], 300_000);
        }
    }["TopBar.useCallback[startPolling]"], [
        processFile
    ]);
    // ── Handler do label/botão Abrir ─────────────────────────────────────────────
    // Inicia o polling ANTES do clique ser repassado ao input pelo browser.
    // O <label htmlFor> garante que o file picker abra nativamente (sem JS).
...
_s(TopBar, "8FRUmRoBj4/QSk9kxn9fiXYfQnM=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$lang$2d$store$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useT"],
...
//# sourceMappingURL=_0e0oo5g._.js.map